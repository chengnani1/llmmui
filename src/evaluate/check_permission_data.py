...
功能：
1. 遍历 processed 目录下所有 fastbot-* 子目录的 results_permission_rule_only.json
2. 检查 predicted_permissions 是否都属于 30 个合法权限集合
3. 将包含 null 的链视为“无权限链”，把 predicted_permissions 改为空列表 []
...
def process_goal_labels(app_dir: str, stats: dict):
    """
    处理单个 fastbot-* 目录下的 results_permission_rule_only.json
    - 更新 stats
    - 修改 null 为“无权限”（predicted_permissions = []）
    """
    label_path = os.path.join(app_dir, "results_permission_rule_llm.json")
    if not os.path.exists(label_path):
        print(f"⚠ 跳过（无 results_permission_rule_only.json）：{app_dir}")
    ...
    if not isinstance(labels, list):
        print(f"⚠ results_permission_rule_only.json 不是列表：{label_path}")
        stats["apps_bad_json"] += 1
        return
...
def main(root_dir: str):
    ...
    stats = {
        ...
        "chains_perm_empty": 0,          # predicted_permissions 为空列表
        "chains_with_permission": 0,     # predicted_permissions 中包含 >=1 个合法权限
    ...
    print(f"  - 其中有 results_permission_rule_only.json 的：{stats['apps_with_label']}")
    print(f"  - 无 results_permission_rule_only.json 的：{stats['apps_no_label']}")
    print(f"  - results_permission_rule_only.json 解析失败的：{stats['apps_bad_json']}")
    ...
    print(f"  - predicted_permissions 不是 list 的条目：{stats['chains_perm_non_list']}")

    print(f"\n  - predicted_permissions 为空（无权限链）：{stats['chains_perm_empty']}")
    ...
    if stats["unknown_perm_counter"]:
        ...
        print("  ✅ 所有 predicted_permissions 均在 30 个合法权限集合中（除 null 已被清空）。")
