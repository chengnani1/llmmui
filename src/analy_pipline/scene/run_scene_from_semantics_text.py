...
def _fallback_from_semantics(chain_id: int, sem: Dict[str, Any], rerun: bool, rerun_reason: str, error: str = "") -> Dict[str, Any]:
    task_phrase = str(sem.get("task_phrase", "")).strip()
    intent = str(sem.get("intent", "")).strip()
    if not intent:
...
def infer_scene(
    ...
    payload = {
        ...
        "keywords": (sem.get("evidence") or {}).get("keywords", []),
        "widgets": (sem.get("evidence") or {}).get("widgets", []),
