# -*- coding: utf-8 -*-
"""
Task13 (12+1 taxonomy) vision scene inference experiment.
"""
...
OUTPUT_FILENAME = "result_scene_task13_vision.json"
SUMMARY_FILENAME = "scene_task13_vision_summary.json"
DEFAULT_PROMPT_FILE = os.path.join(settings.PROMPT_DIR, "scene_task13_vision.txt")
...
def infer_chain_record(
    ...
    try:
        raw = call_vllm_vl(
            build_prompt(prompt_template, input_payload, strict=False, forbid_other=False),
            image_path,
            vllm_url,
            model,
        )
        ...
        raw2 = call_vllm_vl(
            build_prompt(prompt_template, input_payload, strict=True, forbid_other=(reason == "other_low_confidence")),
            image_path,
            vllm_url,
            model,
        )
...
def process_app(app_dir: str, prompt_template: str, vllm_url: str, model: str) -> Tuple[List[Dict[str, Any]], int]:
    ...
    for idx, chain in enumerate(tqdm(chains, desc=f"Scene Task13 Vision {os.path.basename(app_dir)}", ncols=90)):
        ...
        rec = infer_chain_record(chain_id, image_path, input_payload, prompt_template, vllm_url, model)
    ...
    print(f"[SceneTask13-Vision] finish app={app_dir} chains={len(records)} low_conf_other={invalid} out={out_path}")
...
def run(target: str, prompt_file: str, vllm_url: str, model: str) -> None:
    ...
    for app_dir in app_dirs:
        try:
            ...
            print(f"[SceneTask13-Vision][WARN] app failed app={app_dir} err={exc}")
    ...
    print(f"[SceneTask13-Vision] done apps={len(app_dirs)} total_chains={len(all_records)} summary={summary_path}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Task13 vision scene inference experiment")
